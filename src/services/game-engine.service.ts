...
export class GameEngineService {
  ...
  async createQuickMatch(playerId: string): Promise<Game> {
    try {
      ...
      this.logger.log(`Quick match created: ${savedGame.id} by player ${playerId}`);
      return savedGame;

    } catch (error) {
      this.logger.error(`Failed to create quick match for player ${playerId}`, error);
  ...
  async joinGame(gameId: string, playerId: string): Promise<Game> {
    try {
      ...
      this.logger.error(`Failed to join game ${gameId} for player ${playerId}`, error);
   ...
   * @returns Promise<Game> - The updated game
   */
  async submitMove(gameId: string, playerId: string, move: GameMove): Promise<Game> {
    try {
      // Validate move
      ...
      if (updatedGame.isCompleted) {
        ...
        this.logger.log(`Game completed: ${gameId}, winner: ${updatedGame.winner?.id || 'draw'}`);
      }

      return updatedGame;

    } catch (error) {
      this.logger.error(`Failed to submit move for game ${gameId}, player ${playerId}`, error);
   ...
   */
  async getPlayerGames(playerId: string, limit: number = 50, offset: number = 0): Promise<Game[]> {
  ...
  async getPlayerStats(playerId: string): Promise<{
    ...
    // Find favorite move
    const maxMoveCount = Math.max(moveCount.ROCK, moveCount.PAPER, moveCount.SCISSORS);
    if (maxMoveCount > 0) {
      stats.favoriteMove = Object.keys(moveCount).find(
    ...
    stats.recentForm = recentGames.map(game => {
      ...
      if (game.player1?.id === playerId) {
        return game.result === GameResult.PLAYER1_WIN ? 'W' : 
               game.result === GameResult.PLAYER2_WIN ? 'L' : 'D';
      } else {
        return game.result === GameResult.PLAYER2_WIN ? 'W' : 
               game.result === GameResult.PLAYER1_WIN ? 'L' : 'D';
      }
    });
  ...
  async cancelGame(gameId: string, reason?: string): Promise<Game> {
    ...
    this.logger.log(`Game cancelled: ${gameId}${reason ? `, reason: ${reason}` : ''}`);
    return updatedGame;
  }
  ...
  private async updateUserStats(game: Game): Promise<void> {
    ...
    try {
      ...
      const player1Result = game.result === GameResult.PLAYER1_WIN ? 'win' :
                           game.result === GameResult.PLAYER2_WIN ? 'loss' : 'draw';
      
      const player2Result = game.result === GameResult.PLAYER2_WIN ? 'win' :
                           game.result === GameResult.PLAYER1_WIN ? 'loss' : 'draw';
      ...
      // Update stats
      if (game.player1Move) {
        player1Stats.recordGameResult(player1Result, game.player1Move, gameDuration);
      }
      
      if (game.player2Move) {
        player2Stats.recordGameResult(player2Result, game.player2Move, gameDuration);
      }
      ...
      this.logger.error(`Failed to update user stats for game ${game.id}`, error);
  ...
  async processTimedOutGames(): Promise<number> {
    ...
    for (const game of timedOutGames) {
      try {
        ...
        this.logger.error(`Failed to process timeout for game ${game.id}`, error);
      }
    }
