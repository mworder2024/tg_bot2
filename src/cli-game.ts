...
class CLIGame {
  ...
  private async startQuickMatch() {
    ...
    try {
      ...
      // Create game
      const game = await this.gameEngineService.createQuickMatch(
        this.players[0].id,
      );
      console.log(`🎮 Game created: ${game.id}`);

      // Join second player
      const joinedGame = await this.gameEngineService.joinGame(
        game.id,
        this.players[1].id,
      );
  ...
  private async playGame() {
    ...
    console.log(`\\n🎯 Round starting!`);
    console.log(
      `${this.players[0].displayName} vs ${this.players[1].displayName}`,
    );
    ...
    try {
      // Submit moves
      let game = await this.gameEngineService.submitMove(
        this.currentGame.id,
        this.players[0].id,
        moves[0],
      );
      game = await this.gameEngineService.submitMove(
        this.currentGame.id,
        this.players[1].id,
        moves[1],
      );
  ...
  private showFinalResult(game: any) {
    ...
    if (game.winner) {
      const winnerName =
        game.winner.id === this.players[0].id
          ? this.players[0].displayName
          : this.players[1].displayName;
